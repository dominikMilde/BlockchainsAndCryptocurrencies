...
contract Auction {
    ...
    function finalize() public {
        ...
        if(JudgeExists()){ // require TODO
            require(Judge() || WinnerBidderCheck(), "Err");

        }
        finishAuction(Outcome.SUCCESSFUL, highestBidderAddress);
        payable(sellerAddress).transfer(CurrentBalance());
    }
    ...
    function refund() public {
        require(IsNotSuccessful(), "Auction is not unsuccessful");
        require(WinnerBidderExists(), "Highest bidder not present");
        if (JudgeExists()) {
            require(Judge() || Seller(), "Can only be invoked by judge or seller");
        }
    ...
    function IsNotSuccessful() internal view returns (bool){
        if(outcome == Outcome.NOT_SUCCESSFUL) return true;
        return false;
    }
    function WinnerBidderExists() internal view returns (bool){
        if(highestBidderAddress != address(0)) return true;
        return false;
    }
    function JudgeExists() internal view returns (bool){
        if(judgeAddress != address(0)) return true;
        return false;
    }
    function Judge() internal view returns (bool){
        if(judgeAddress == msg.sender) return true;
        return false;
    }
    function Seller() internal view returns (bool){
        if(sellerAddress == msg.sender) return true;
        return false;
    }
    function IsSuccessful() internal view returns (bool){
        if(outcome == Outcome.SUCCESSFUL) return true;
        return false;
    }
    ...
    function WinnerBidderCheck() internal view returns (bool){
        if(highestBidderAddress == msg.sender) return true;
        return false;
    }
    function NotFinished() internal view returns (bool){
        if(outcome == Outcome.NOT_FINISHED) return true;
        return false;
    }
}
